...
resource seeds and fragments use the versioned `material_variant` data component, so a datapack or
another Nero mod can introduce a material without creating another registered item.

## Core content

- Generic `resource_seed` and `resource_fragment`, five neutral fragments, and blank/charged seeds.
...
- Fragment Extractor, Fragment Infuser, Seed Synthesizer, Planter, and Harvester.
- Stable shells for food and alien crops, research, greenhouses, fertiliser, genetics, oxygen,
  biofuel, crop towers, pollination, compacted fragment/decor, and terraforming.
