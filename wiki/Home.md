# NeroAgriculture Wiki

Player- and contributor-facing documentation for **NeroAgriculture**, part of the
[Neroland ecosystem](../../neroland-mc-ecosystem/README.md). Built on **Neroland Core**.

> **Status:** resource-seed economy rework. NeroAgriculture is fully playable with **Neroland Core
> alone**; every other Nero mod adds optional links that are dormant-safe when absent.

## The core loop

NeroAgriculture is a *resource farm*: **craft a resource seed → grow it → harvest the resource**.

1. **Extract** a resource with the **Fragment Extractor** to get **Tier Fragments** (the ladder
   currency) plus a component-tinted **Resource Fragment**.
2. **Upgrade** Tier Fragments up the five-step **Cosmic-ascent ladder** — Territe → Forgite → Orbite →
   Colonite → Voidite — in the **Fragment Infuser** (NeroFlux-powered). Producing each tier opens the
   next native progression gate, so the whole ladder is reachable standalone.
3. **Craft a Prospora Seed** (the base item) and synthesize a **Resource Seed** in the Seed Synthesizer
   from *the real resource + matching Tier Fragments + a Prospora Seed*.
4. **Grow** the Resource Seed on a tier grow bed; harvest **Resource Fragments**; convert **N
   Fragments → the resource**.
5. **Fuse** two resources into an **alloy seed** (e.g. Iron Fragment + Coal → Steel) in the Infuser.

See [Resource-seed economy](Resource-Seed-Economy.md) for the full walkthrough.

## Contents

- [Resource-seed economy](Resource-Seed-Economy.md)
- [Fusion and alloy seeds](Fusion-and-Alloy-Seeds.md)
- [Sibling-mod progression overlays](Sibling-Overlays.md)
- [Nerospace integration (planet visits)](Nerospace-Integration.md)
- [Material catalog and crop storage](Material-Catalog-and-Crop-Storage.md)
- [Resource crops](Resource-Crops.md)
- [Resource fragment](Resource-Fragment.md)
- [Grow beds](Grow-Beds.md)
- [Growth conditions](Growth-Conditions.md)
- [Fabrication chain](Fabrication-Chain.md)
- [Seed research](Seed-Research.md)
- [Material recipe datapacks](Material-Recipe-Datapacks.md)
- [Automation (Planter & Harvester)](Automation.md)
- [Automated crop towers](Crop-Towers.md)
- [Fertiliser](Fertiliser.md)
- [Side configuration](Side-Configuration.md)
- [Genetics](Genetics.md)
- [Breeding and mutations](Breeding.md)
- [Pollination](Pollination.md)
- [Biofuel and fragment blocks](Biofuel-and-Fragment-Blocks.md)
- [Seasonal and stellar cycles](Seasonal-Cycles.md)
- [Terraforming](Terraforming.md)
- [Compatibility](Compatibility.md)
- [Art and visuals](Art-and-Visuals.md)
- [Configuration](Configuration.md)
- [Administration](Administration.md)
- [Performance](Performance.md)
- [Privacy and erasure](Privacy-and-Erasure.md)
- [FAQ](FAQ.md)

### Food & alien crops (secondary side-system)

Food and alien farming is a self-contained optional branch — not on the resource-progression critical
path. See [Engineered foods](Engineered-Foods.md), [Food effects](Food-Effects.md),
[Alien crops](Alien-Crops.md), [Planet varieties](Planet-Varieties.md),
[Greenhouse construction](Greenhouse-Construction.md),
[Greenhouse troubleshooting](Greenhouse-Troubleshooting.md), and
[Oxygen and life support](Oxygen-and-Life-Support.md).

## See also

- [Build & contributor context](../AGENTS.md)
- [Ecosystem overview](../../neroland-mc-ecosystem/README.md)
- [This mod's planning docs](../../neroland-mc-ecosystem/neroagriculture/)

## Stage history

- [Stage 2 foundation and stable IDs](Stage-2-Foundation.md)
