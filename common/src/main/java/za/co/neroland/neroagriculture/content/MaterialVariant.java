...
/** Versioned dynamic material identity stored on finite registered seed/essence items. */
public record MaterialVariant(int version, Identifier material, EssenceFamily family) {
    ...
    private static final Codec<MaterialVariant> RAW_CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            EssenceFamily.CODEC.fieldOf("family").forGetter(MaterialVariant::family)
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, MaterialVariant> STREAM_CODEC = StreamCodec.of(
            ...
            buffer -> checked(buffer.readVarInt(), Identifier.parse(buffer.readUtf(MAX_ID_LENGTH)),
                    buffer.readEnum(EssenceFamily.class)));

    public static MaterialVariant of(Identifier material, EssenceFamily family) {
        return checked(CURRENT_VERSION, material, family);
    }

    private static MaterialVariant checked(int version, Identifier material, EssenceFamily family) {
    ...
    private static DataResult<MaterialVariant> validate(MaterialVariant value) {
        ...
        if (value.family == null) return DataResult.error(() -> "Missing essence family");
