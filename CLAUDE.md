# Project context for AI coding agents — neroagriculture

> This and `AGENTS.md` are kept identical; update both together.

## The mod

- **NeroAgriculture** — part of the Neroland sci-fi Minecraft mod ecosystem, built on **Neroland Core**.
...
- Mod id: **`neroagriculture`** (matches the registry namespace + every loader manifest). Package root:
  `za.co.neroland.neroagriculture`. Author: **Neroland**.
...
- Loader entry points: `NeroAgricultureFabric` (+ `NeroAgricultureFabricClient`), `NeroAgricultureForge`,
  `NeroAgricultureNeoForge` — each calls `NeroAgricultureCommon.init()` during construction.
- NeoForge/Forge debug tasks use `-PneroagricultureDebug`; Fabric Loom honours Gradle `--debug-jvm`.

## IDE (VS Code) run & debug

- Workspace: **`neroagriculture.code-workspace`** (single-root `"."`). Import the Stonecutter nodes as **static
  ...
  VS Code. Per-node Eclipse project names are `neroagriculture-<loader>-<mc>`.
...
- This mod has its own **dedicated wiki** in `wiki/` at the repo root: the player- and
  contributor-facing docs for NeroAgriculture (features, blocks/items, machines, progression, recipes, FAQ).
...
- The wiki is **per-mod** — document only NeroAgriculture here; cross-mod / ecosystem concepts belong in the
