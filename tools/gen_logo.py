#!/usr/bin/env python3
"""
Generate the NEROAGRICULTURE mod logo (square), in the shared Neroland family style
...
the farming/terraforming mod, so the core-prism is framed by a GREEN grain-leaf laurel and a wheat sheaf Renders supersampled, then downsamples.

Outputs:
  art/logo/neroagriculture_logo.png       (1024x1024 master)
  art/logo/neroagriculture_logo_400.png   (CurseForge/Modrinth-ready)
  common/src/main/resources/neroagriculture_logo.png  (256x256 in-game mods-list icon)
"""
SEED = 71
MOD = 'neroagriculture'
NAME = 'NEROAGRICULTURE'
ACCENT = (120, 200, 96)
ACCENT_BRIGHT = (188, 240, 158)
BRIGHT = (236, 252, 228)
NEBULA = (40, 104, 40)
STAR_TINT = (200, 244, 180)
NAME_GLOW = (120, 200, 96, 255)
TAG_COL = (190, 222, 178, 255)
...
TAG = 'F A R M I N G   ·   C R O P S   ·   H A R V E S T'

def emblem_frame(base, cx, cy, rad):
    # laurel / grain ring: paired leaves sweeping up both sides
    gl = Image.new("RGBA", (R, R), (0, 0, 0, 0)); gd = ImageDraw.Draw(gl)
    ring = rad * 1.6
    for sgn in (-1, 1):
        for i in range(7):
            a = math.radians(200 - i * 22) if sgn < 0 else math.radians(-20 + i * 22)
            bx, by = cx + math.cos(a) * ring, cy + math.sin(a) * ring
            la = a + sgn * math.radians(28)
            lx, ly = bx + math.cos(la) * rad * 0.5, by + math.sin(la) * rad * 0.5
            leaf = [(bx, by),
                    (bx + math.cos(la + 0.5) * rad * 0.34, by + math.sin(la + 0.5) * rad * 0.34),
                    (lx, ly),
                    (bx + math.cos(la - 0.5) * rad * 0.34, by + math.sin(la - 0.5) * rad * 0.34)]
            gd.polygon(leaf, fill=ACCENT + (235,))
    base.alpha_composite(gl.filter(ImageFilter.GaussianBlur(SS)))
    base.alpha_composite(gl)
    # wheat sheaf at the base
    wl = Image.new("RGBA", (R, R), (0, 0, 0, 0)); wd = ImageDraw.Draw(wl)
    by = cy + rad * 1.95
    for dx in (-rad * 0.42, 0, rad * 0.42):
        wd.line([(cx + dx * 0.3, by), (cx + dx, by - rad * 1.0)], fill=ACCENT + (255,), width=SS * 3)
        for gy in range(4):
            yy = by - rad * 0.35 - gy * rad * 0.2
            xx = cx + dx * (0.55 + gy * 0.13)
            wd.ellipse([xx - rad * 0.1, yy - rad * 0.16, xx + rad * 0.1, yy + rad * 0.16],
